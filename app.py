...
if "M2_D" in view:
    ...
    yoy   = (month.pct_change(12)*100).rename("YoY")
    def cls(x):
...
def card(title, val, code):
    ...
    return f"""<div style="background:{bg};border-radius:8px;padding:20px 12px;text-align:center;color:#fff;">
...
# ── 6. 섹션 선택 (Selectbox — 구버전 호환) ─────────────────────
section = st.selectbox("그래프·표 선택",
...
# Signal 표
    ...
    tbl = {k: v for k, v in sigs.items() if v}
