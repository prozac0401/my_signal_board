...
# KODEX200 열 확보 ─ 없으면 069500.KS 로 대체
if "KODEX200" not in df.columns and "069500.KS" in df.columns:
...
def card(title, val, code):
    ...
    <div style="background:{bg};border-radius:8px;padding:20px 12px;text-align:center;color:white;">
      ...
      <div style="font-size:32px;font-weight:700;line-height:1.1;margin:4px 0;">{val_fmt}</div>
...
if "Gold_KRWg" in view:
    c1.markdown(card("Gold (원/g)", view["Gold_KRWg"].iloc[-1], latest(sig_gold)),
                unsafe_allow_html=True)
if "KODEX200" in view:
    c2.markdown(card("KODEX 200",  view["KODEX200"].iloc[-1],  latest(sig_kdx)),
                unsafe_allow_html=True)
if "FX" in view:
    c3.markdown(card("USD/KRW",    view["FX"].iloc[-1],        latest(sig_fx)),
                unsafe_allow_html=True)
