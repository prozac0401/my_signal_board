...
# ── 5. 카드 HTML ───────────────────────────────
def card(title: str, val: float, code: int) -> str:
    bg, tag = COLOR[code], LABEL[code]
    val_fmt = f"{val:,.0f}" if val > 1000 else f"{val:,.2f}"
    return f"""
    ...
    for d, c in sig[sig.shift(1) != sig].items():
        if c:
            yield dict(
                ...
                y0=0,
                y1=1,
                line=dict(color=COLOR[c], width=1, dash="dot"),
                opacity=0.25,
            )
...
if "Gold_KRWg" in view:
    cols[0].markdown(
        card("Gold (원/g)", view["Gold_KRWg"].iloc[-1], latest(sig_gold)),
        unsafe_allow_html=True,
    )
if "KODEX200" in view:
    cols[1].markdown(
        card("KODEX 200", view["KODEX200"].iloc[-1], latest(sig_kdx)),
        unsafe_allow_html=True,
    )
if "FX" in view:
    cols[2].markdown(
        card("USD/KRW", view["FX"].iloc[-1], latest(sig_fx)),
        unsafe_allow_html=True,
    )

tab_gold, tab_kodex, tab_fx, tab_signal = st.tabs(
    ["금 가격", "KODEX 200", "환율", "Signal"]
)
...
with tab_signal:
    ...
    if sig_table:
        st.table(pd.Series(sig_table, name="Signal").to_frame())
    else:
        st.info("최근 비중 변화 신호가 없습니다.")

st.caption(
    "Data sources: FRED · Stooq · ECOS · Yahoo Finance  |  Signals = 20/50 MA cross"
)
