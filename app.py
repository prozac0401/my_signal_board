...
| **멀티 컨펌** | M2 YoY > 0 + 환율 ↓ + KODEX 200 ↑ → **공격적 비중 확대**<br>M2 YoY ≤ 0 + Gold ↓ → 안전자산 축소·현금 확보 |
...
# M2 YoY% → 신호 (0 선 돌파)
if "M2_D" in view:
    m_month = view["M2_D"].resample("M").last()
    m_yoy   = (m_month.pct_change(12)*100).rename("M2_YoY")
    s_m2 = (m_yoy > 0).astype(int).replace({0:-1})   # >0 🟢, ≤0 🔴
    # 월 신호를 일 범위로 forward-fill
    s_m2 = s_m2.reindex(view.index, method="ffill")
else:
    s_m2 = pd.Series()
...
with tab_m2:
    if "M2_D" in view:
        ...
        st.plotly_chart(px.line(m_month, title="M2 월말 값 · MA6·12"), use_container_width=True)

        # YoY % + 선
        yoy = (m_month["M2_M"].pct_change(12)*100).rename("YoY%")
        fig = px.bar(yoy, title="M2 YoY 증가율 (%)", opacity=0.4,
                     labels={"value":"%", "index":"date"})
        fig.add_scatter(x=m_month.index, y=m_month["M2_M"], mode="lines",
                        name="M2 (월)", yaxis="y2")
