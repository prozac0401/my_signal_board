...
if "M2_D" in view:
    ...
    yoy     = (m_month.pct_change(12) * 100).rename("YoY")
    def classify(x):
        if pd.isna(x):   return -1   # 초기
        if x >  9:       return  2
        if x >= 6:       return  1
        if x >= 3:       return -1
        return -2
...
with tab_kdx:
    if "KODEX200" in view:
        ...
        st.warning("KODEX 200 데이터가 없습니다.")

# M2 그래프 (YoY 막대 + 월말/MA6/MA12 라인)
with tab_m2:
    if "M2_D" in view:
...
with tab_sig:
    ...
    tbl = {k: v for k, v in signals.items() if v}
