...
| 구간 | 해석 | 시사점 |
|------|------|-------|
| **> 9 %** <span style="color:#16a085">● 팽창</span> | 장기 평균+0.5 σ 이상 | **리스크-온** : 주식·부동산·원자재 확대 |
| **6 – 9 %** <span style="color:#2ecc71">● 완충</span> | 평균 부근 | 중립·추세 확인 |
| **3 – 6 %** <span style="color:#f39c12">● 둔화</span> | 평균-1 σ ↑ | 경계, 포트 리밸런스 |
| **< 3 %** <span style="color:#e74c3c">● 수축</span> | 평균-1 σ 이하 | **리스크-오프** : 현금·단기채 ↑ |

#### ✨ 응용 TIP
| 아이디어 | 설명 |
|----------|------|
| **신호 필터링** | M2 YoY > 9 % 구간일 때만 주식 신호(**KODEX 200**) 채택 → 가짜 반등 회피 |
| **멀티 컨펌** | M2 팽창(> 9 %) + 환율 ↓ + KODEX 200 ↑ → **공격적 비중 확대**<br> M2 수축(< 3 %) + 금 ↓ → 안전자산 축소·현금 확보 |
| **모멘텀 결합** | ‘팽창’ 구간이면서 M2 YoY 20 EMA 위·기울기 ↑ 일 때만 리스크-온 |
"""
...
if "M2_D" in view:
    ...
    yoy = (m.pct_change(12)*100).rename("M2_YoY")
    def classify(x):
        if pd.isna(x): return -1   # 초기값
        if x >  9: return  2
        if x >= 6: return  1
        if x >= 3: return -1
        return -2
    s_m2 = yoy.apply(classify).reindex(view.index, method="ffill")
else:
    s_m2 = pd.Series()

# ── 5. 컬러 & 라벨 ──────────────────────────────────────────────
COL = { 2:"#16a085", 1:"#2ecc71", -1:"#f39c12", -2:"#e74c3c", 0:"#4d4d4d"}
...
with tab_m2:
    if "M2_D" in view:
        ...
        fig = px.bar(yoy, title="M2 YoY 증가율 (%)", opacity=0.4)
